...
class DiffILOGNNPolicy(torch.nn.Module):
    def __init__(self, emb_size=32, cons_nfeats=3, edge_nfeats=1, var_nfeats=5, depth=6):
        super().__init__()
        ...
        self.cons_embedding = nn.Sequential(
            ...
            nn.Linear(cons_nfeats, self.emb_size),
            nn.ReLU(),
        ...
        self.var_embedding = nn.Sequential(
            ...
            nn.Linear(var_nfeats, self.emb_size),
            nn.ReLU(),
            nn.Linear(self.emb_size, self.emb_size),
            nn.ReLU(),
        )
        ...
        for _ in range(self.depth):
            self.conv_v_to_c_layers.append(BipartiteGraphConvolution(self.emb_size))
            self.graph_norm_v_to_c_layers.append(GraphNorm(self.emb_size))
            self.conv_c_to_v_layers.append(BipartiteGraphConvolution(self.emb_size))
            self.graph_norm_c_to_v_layers.append(GraphNorm(self.emb_size))


        self.output_norm = GraphNorm((self.depth + 1) * self.emb_size)
        self.vars_output_layer = torch.nn.Sequential(
            nn.Linear((self.depth + 1) * self.emb_size, 512),
            nn.ReLU(),
            nn.Linear(512, 128),
            nn.ReLU(),
        ...
        self.cons_output_layer = torch.nn.Sequential(
            nn.Linear((self.depth + 1) * self.emb_size, self.emb_size),
            nn.ReLU(),
            nn.Linear(self.emb_size, 1) #, bias=True),
        )
