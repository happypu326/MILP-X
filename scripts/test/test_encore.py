...
from src.evaluator.encore_evaluator import EnCoreEvaluator
...
def build_model(cfg_dict, device):
    ...
    model = build_ps_family_model(
        ...
        variable_nfeats=cfg_dict["variable_nfeats"],   # 7 for EnCore
...
def build_eval_config(cfg_dict, device, worker_id, worker_log_root,
                      worker_result_root, worker_scores_root, chunk_size):
    return {
        ...
        'probe_time': cfg_dict.get("probe_time", 20),
        'encore_k': cfg_dict.get("encore_k", 3),
        'test_num': chunk_size,
    }


def main():
    cfg = OmegaConf.load('configs/test/test_encore.yaml')
