...
from src.evaluator.diffusion_ps_evaluator import DiffusionPSEvaluator
...
def build_model(cfg_dict, device):
    model = DiffusionGNNPolicy(
        ...
        depth=cfg_dict.get("depth", 4),
        jumping_knowledge=cfg_dict.get("jumping_knowledge", True),
    ).to(device)
...
def build_eval_config(cfg_dict, device, worker_id, worker_log_root, worker_result_root, worker_scores_root, chunk_size):
    return {
        ...
        'num_timesteps': cfg_dict["num_timesteps"],
        'inference_steps': cfg_dict["inference_steps"],
        'n_samples': cfg_dict["n_samples"],
        'schedule': cfg_dict["schedule"],
        'feasibility_projection': cfg_dict.get("feasibility_projection", False),
        'proj_steps': cfg_dict.get("proj_steps", 5),
        'proj_lr': cfg_dict.get("proj_lr", 0.1),
        'test_num': chunk_size,
    }


def main():
    cfg = OmegaConf.load('configs/test/test_diffusion.yaml')
    ...
    run_parallel_eval(
        cfg_dict=cfg_dict,
        instances=instances,
        evaluator_cls=DiffusionPSEvaluator,
