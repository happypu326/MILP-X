...
from src.evaluator.guided_diffusion_evaluator import GuidedDiffusionEvaluator
...
def build_model(cfg_dict, device):
    model = GuidedDiffusionModel(
        emb_size=cfg_dict["emb_size"], latent_dim=cfg_dict["latent_dim"],
        constraint_nfeats=cfg_dict["constraint_nfeats"], edge_nfeats=cfg_dict["edge_nfeats"],
...
def build_eval_config(cfg_dict, device, worker_id, worker_log_root, worker_result_root, worker_scores_root, chunk_size):
    return {
        ...
        'num_timesteps': cfg_dict["num_timesteps"], 'n_samples': cfg_dict["n_samples"],
        'feasibility_projection': cfg_dict.get("feasibility_projection", False),
        'proj_steps': cfg_dict.get("proj_steps", 5), 'proj_lr': cfg_dict.get("proj_lr", 0.1),
        'test_num': chunk_size,
    }


def main():
    cfg = OmegaConf.load('configs/test/test_guided_diffusion.yaml')
    ...
    run_parallel_eval(cfg_dict=cfg_dict, instances=instances, evaluator_cls=GuidedDiffusionEvaluator,
